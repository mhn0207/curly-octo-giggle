...
from core.structured_schemas import JudgeOutput
...
评分必须以提供的背景为依据。背景不足时，不应因 Agent 谨慎说明无法确认而扣分；如果 Agent 在缺少依据时声称已查询、已处理、必然成功或固定到账，应显著降低 accuracy 和 helpfulness。"""
    ...
    async def judge(
        ...
        task_prompt = self._clean_text(self.JUDGE_PROMPT.format(
            question=question,
            response=response,
        ...
        async def legacy_call() -> JudgeOutput:
    ...
    def structured_output_stats(self) -> Dict[str, Any]:
        if self._structured_invoker is None:
            return {"llm_judge": {"mode": "legacy"}}
        return {"llm_judge": self._structured_invoker.stats_snapshot()}
...
class EndToEndEvaluator:
    ...
    def __init__(
        ...
        judge_invoker = create_structured_invoker(
            api_key=api_key,
            model=model,
            base_url=base_url,
            component="llm_judge",
        ...
        self._judge = LLMJudge(client, model, judge_invoker)
    ...
    def structured_output_stats(self) -> Dict[str, Any]:
        return self._judge.structured_output_stats()
